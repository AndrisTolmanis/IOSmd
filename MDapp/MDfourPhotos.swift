...
class MDfourPhotos: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: "customcell", for: indexPath) as! ListCellPhotos
            ...
            if error != nil{
                ...
                if let json = try? JSON(data: data!){
                    for (_,subJson):(String, JSON) in json {
                        if subJson["name"].string!.hasSuffix(".jpg") ||
                            subJson["name"].string!.hasSuffix(".png"){
...
class ListCellPhotos: UITableViewCell{
